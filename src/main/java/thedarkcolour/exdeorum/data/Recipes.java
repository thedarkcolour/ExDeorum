...
class Recipes {
    ...
    private static void lavaCrucible(Consumer<FinishedRecipe> consumer, String id, Ingredient ingredient, int volume) {
        consumer.accept(new FinishedCrucibleRecipe(new ResourceLocation(ExDeorum.ID, "lava_crucible/" + id), ERecipeSerializers.LAVA_CRUCIBLE.get(), ingredient, Fluids.LAVA, volume));
    }

    private static void waterCrucible(Consumer<FinishedRecipe> consumer, String id, Ingredient ingredient, int volume) {
        consumer.accept(new FinishedCrucibleRecipe(new ResourceLocation(ExDeorum.ID, "water_crucible/" + id), ERecipeSerializers.WATER_CRUCIBLE.get(), ingredient, Fluids.WATER, volume));
    }
    ...
    private static void hammerRecipe(Consumer<FinishedRecipe> consumer, String name, Ingredient block, ItemLike result, NumberProvider resultAmount) {
        consumer.accept(new FinishedHammerRecipe(new ResourceLocation(ExDeorum.ID, "hammer/" + name), block, result.asItem(), resultAmount));
    }
    ...
    private static void barrelMixing(Consumer<FinishedRecipe> writer, Ingredient ingredient, Supplier<FluidType> fluidType, Item result) {
        writer.accept(new FinishedBarrelMixingRecipe(new ResourceLocation(ExDeorum.ID, "barrel_mixing/" + path(result)), ingredient, fluidType.get(), 1000, result));
    }
